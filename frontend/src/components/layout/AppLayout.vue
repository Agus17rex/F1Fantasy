<template>
  <div class="flex h-screen overflow-hidden bg-zinc-950">
    ...
    <aside
      :class="[
        'fixed lg:relative inset-y-0 left-0 z-40 w-64 flex-shrink-0 bg-zinc-900 border-r border-zinc-800 flex flex-col transition-transform duration-300',
        sidebarAbierto ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      ]"
    >
      <!-- Logo -->
      <div class="h-16 flex items-center px-6 border-b border-zinc-800 gap-2">
        ...
        <!-- Cerrar en móvil -->
        <button class="ml-auto lg:hidden text-zinc-400 hover:text-white" @click="sidebarAbierto = false">
      ...
      <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        <RouterLink to="/" custom v-slot="{ navigate }">
          <button @click="navegar(navigate)" :class="route.name === 'Dashboard' ? 'nav-link-active' : 'nav-link'" class="w-full">
            <IconHome /> Dashboard
          </button>
        </RouterLink>
        ...
        <RouterLink to="/reglas" custom v-slot="{ isActive, navigate }">
          <button @click="navegar(navigate)" :class="isActive ? 'nav-link-active' : 'nav-link'" class="w-full">
            <IconBook /> Reglas
          </button>
        </RouterLink>
    ...
    <main class="flex-1 overflow-y-auto min-w-0">
      ...
      <header class="h-16 bg-zinc-900 border-b border-zinc-800 flex items-center px-4 sticky top-0 z-20">
        ...
        <h1 class="text-base lg:text-lg font-semibold text-white truncate">{{ pageTitle }}</h1>
        <div class="ml-auto flex items-center gap-2 flex-shrink-0">
          <span v-if="nextRace" class="hidden sm:block text-xs text-zinc-400">
            Próxima: <span class="text-red-400 font-medium">{{ nextRace.nombre }}</span>
          </span>
        </div>
      </header>
...
const IconBook    = { template: `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>` }
...
onMounted(() => f1Store.fetchProximaCarrera())
</script>
